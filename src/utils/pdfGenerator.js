 ...
 * @description Client-side PDF generation for all HSClogic FMS document types.
 *
 * All functions use jsPDF directly (no html2canvas) for precise layout control
 * and consistent output across browsers. Documents are saved immediately to
 * the user's downloads folder.
 *
 * Exported functions:
 *   generateInvoicePDF(invoice)             — branded A4 invoice
 *   generatePayslipPDF(record, period)      — per-employee payslip
 *   generateSalaryReportPDF(employees, fn)  — full salary structure summary
 *
 * The generatePayrollReportPDF function is co-located in PayrollPage.jsx
 * because it is only used in that module and dynamically imports jsPDF to
 * avoid a duplicate static import.
 */
import jsPDF from "jspdf";

// Shared colour palette for white-background professional document output
const C = {
  black:   [15,  17,  23],
  dark:    [40,  45,  55],
  mid:     [90, 100, 115],
